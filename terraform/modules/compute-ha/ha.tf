...
resource "google_compute_instance_template" "it-osi-pi-daaf" {
  ...
  project        = var.project_id
  machine_type   = lookup(var.compute-machine-type,var.epsec,10000)
  tags           = ["osi-internal","health-check"]
  ...
  network_interface {
    network    = var.vpc
    subnetwork = lookup(var.compute-multi-subnets,count.index+6)
  }
...
resource "google_compute_address" "tcpilb_address" {
  ...
  subnetwork   = lookup(var.compute-multi-subnets,8)
  address_type = "INTERNAL"
  project      = var.project_id
  region       = var.compute-region
}
...
resource "google_compute_forwarding_rule" "fwd-daaf" {
  ...
  ports                 = ["5457","5450"]
  network               = var.vpc
  subnetwork            = lookup(var.compute-multi-subnets,count.index+8)
  service_label         = "daaf"
}
...
resource "google_compute_address" "ani_address" {
  count = var.architecture == "Non-HA" ? 0 : 5
...
resource "google_compute_instance_template" "it-osi-pi-ani" {
  ...
  metadata = {
    ...
    an2                        = "${google_compute_address.ani_address1.0.address}"
    ipWSFC1                    = "${google_compute_address.ani_address.1.address}"
    ipWSFC2                    = "${google_compute_address.ani_address1.2.address}"
    iscsi                      = "${google_compute_address.ani_address.3.address}"
    IPClusRole1                = "${google_compute_address.ani_address.4.address}"
    IPClusRole2                = "${google_compute_address.ani_address1.1.address}"
...
resource "google_compute_instance_template" "it-osi-pi-ani1" {
  ...
  network_interface {
    ...
    network_ip = google_compute_address.ani_address1.0.address
  }
...
resource "google_compute_forwarding_rule" "fwd-ani" {
  ...
  region                = var.compute-region
  load_balancing_scheme = "INTERNAL"
  ip_address            = element(google_compute_address.tcpilb_address1.*.address,count.index)
  ip_protocol           = "TCP"
  backend_service       = element(google_compute_region_backend_service.bk-ani.*.id,count.index)
  ports                 = ["5463","5468"]
  network               = var.vpc
  subnetwork            = lookup(var.compute-multi-subnets,count.index+9)
  service_label         = "ani"
}
...
resource "google_compute_instance" "pi-witness" {
  ...
  depends_on = [google_compute_forwarding_rule.fwd-ani]
}
...
resource "google_compute_instance_template" "it-pivii" {
  ...
  region         = var.compute-region
  machine_type   = lookup(var.compute-machine-type,var.epsec,10000)
  tags           = ["osi-internal","osi-web","health-check"]
  can_ip_forward = false
...
resource "google_compute_instance_template" "it-womf" {
  ...
  disk {
    // Instance Templates reference disks by name, not self link
    # device_name = "${element(google_compute_disk.disk-osi1.*.name,count.index+2)}"
    mode        = "READ_WRITE"
    auto_delete = false
    boot        = false
    type         = "pd-standard"
...
#############################################
resource "google_compute_http_health_check" "hc-pivii" {
  count = var.architecture == "Non-HA" ? 0 : 1
...
##########################################
resource "google_compute_http_health_check" "hc-womf" {
  count = var.architecture == "Non-HA" ? 0 : 1
...
resource "google_compute_backend_service" "bk-pivii" {
  ...
  timeout_sec                     = 10
  connection_draining_timeout_sec = 10
  health_checks                   = [google_compute_http_health_check.hc-pivii.0.id]
...
resource "google_compute_backend_service" "bk-womf" {
  ...
  timeout_sec                     = 10
  connection_draining_timeout_sec = 10
  health_checks                   = [google_compute_http_health_check.hc-womf.0.id]
  security_policy                 = var.security_policy
...
resource "google_compute_url_map" "um-pivii" {
  ...
  path_matcher {
    ...
    path_rule {
      paths   = ["/PIVision"]
      service = google_compute_backend_service.bk-pivii[count.index].id
    }

    path_rule {
      paths   = ["/piwebapi"]
      service = google_compute_backend_service.bk-womf[count.index].id
    }
  }
